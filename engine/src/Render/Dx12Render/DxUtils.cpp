    ...
    Microsoft::WRL::ComPtr<ID3D12Resource>& uploadBuffer)
{
    ...
    auto toCopyDestBarrier = CD3DX12_RESOURCE_BARRIER::Transition(defaultBuffer.Get(), D3D12_RESOURCE_STATE_COMMON,
                                                                  D3D12_RESOURCE_STATE_COPY_DEST);
	cmdList->ResourceBarrier(1, &toCopyDestBarrier);

    UpdateSubresources<1>(cmdList, defaultBuffer.Get(), uploadBuffer.Get(), 0, 0, 1, &subResourceData);
	
    auto toReadBarrier = CD3DX12_RESOURCE_BARRIER::Transition(defaultBuffer.Get(), D3D12_RESOURCE_STATE_COPY_DEST,
                                                              D3D12_RESOURCE_STATE_GENERIC_READ);
	...
	const std::string& target)
{
	...
	hr = D3DCompileFromFile(wfilename.c_str(), defines, D3D_COMPILE_STANDARD_FILE_INCLUDE,
